...
uint8 HalI2CRead(uint8 len, uint8 *pBuf)
{
  ...
  if (i2cMstStrt(I2C_MST_RD_BIT) != mstAddrAckR) // 0x40
  {
    len = 0; 
  }
  ...
  while (len > 0)
  {
    ...
    if (I2CSTAT != mstDataAckR) // 0x50
    {
      if (I2CSTAT != mstDataNackR) // 0x58
      {
...
{
  if (i2cMstStrt(0) != mstAddrAckW)  // 0x18
  {
    len = 0; 
  }
  ...
  for (uint8 cnt = 0; cnt < len; cnt++)
  {
    ...
    if (I2CSTAT != mstDataAckW)  // 0x28
    {
      if (I2CSTAT == mstDataNackW) // 0x30
      {
        len = cnt + 1;
      }
      else
