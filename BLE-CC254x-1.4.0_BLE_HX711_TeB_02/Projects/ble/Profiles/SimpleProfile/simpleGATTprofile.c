    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR3_UUID:
        ...
        if ( offset == 0 )
        {
			...
			#else	//对方(Central)用串口发送数据个数的随意的(暂时人为约定小于等于20 SIMPLEPROFILE_CHAR1_LEN)
					//若＞20就任务错了，因为我的attribute table中的buffer只有 SIMPLEPROFILE_CHAR1_LEN 大
					//(也可以不用判断，我只接收20个一次)
					//还可以在Central端发送的时候判断下，多于20个就不发并且提示错误 这样比发过来接收了不处理更好
					//还可以在Central端把多于20字节的情况分多次发送 暂时不考虑这种情况
			if ( len > SIMPLEPROFILE_CHAR1_LEN )   //len 等于多少是由谁决定的 ，是simpleProfileAttrTbl之characteristic 1之Characteristic Value长度决定的  (错)
			{				  						//为什么不是其他 characteristic  (错)
				status = ATT_ERR_INVALID_VALUE_SIZE;
			}
